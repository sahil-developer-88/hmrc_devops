...
import json
import random
from .models import mtd_tokens,posted ,testuser, apps_ids, log1, auth_users
...
class a_testagent(View):  #testagent code
    def get(self, request):
        ...
        post_Data2={
            "serviceNames":     ["agent-services"] # ["mtd-vat"]
        }
        headers ={
        ...
        if r.status_code == 403:
            l1=log1(src_sys_id='Testuser', status1='403 error' , #+ str(e),
            ...
            return render (request,  'pages/mtd/apis/tuagent.html',
                    ...
                    'pd2':post_Data2,
                    'headers1': headers,
                    'title': 'AgentCreate: FAIL' + str(r.status_code) + ' ' + str(r.reason),
                    'clientid': obj.client_id,
                    'mtd_url': mtd_url,
                    'location':'a_testagent' })	
        elif r.status_code == 201:
            response_vat=r.json()  #next refctor to rj
            t2=testuser(endpoint=endpoint,  #next step create agent table and reflects all API fields
                #vrn=response_vat['vrn'],
                vrn=response_vat['agentServicesAccountNumber'],
                userid= response_vat['userId'],
                userFullName=response_vat['userFullName'],
                emailAddress=response_vat['emailAddress'],
                password=response_vat['password'],
                src_sys_id='create_agent',
                appname=obj.appname 
                )
            t2.save()
            ...
            l1.save()
            return render (request,  'pages/mtd/apis/tuagent.html',
                    {'vat': 'Create testagent'	,
                    'endpoint':  'user: ' + response_vat['userId'] + ' pwd:  ' + response_vat['password'],
                    'pd2':  str(r.status_code) + '  ' + r.reason,  #post_Data2,
                    'headers1': response_vat['agentServicesAccountNumber'] , #headers,
                    'title': 'AgentCreate: PASS',
                    'clientid': obj.client_id,
                    'mtd_url': mtd_url })
        else:
            a=1
            #do nothing? do we still need else: here?
...
class a_last30(View):
    def post(self, request):
        ...
        if 'upload' in request.POST:
            ...
            agents=mtd_tokens.objects.raw('select 1 as id, a.vrn, b."userFullName" from pages_mtd_tokens a join pages_testuser b on a.vrn=b.vrn group by 1,a.vrn, b."userFullName"')
            vrn= request.POST.get('AGENT')
            agents_name=mtd_tokens.objects.raw('select 1 as id, a.vrn, b."userFullName" as ufn from pages_mtd_tokens a join pages_testuser b on a.vrn=b.vrn  where a.vrn=\''+ vrn+'\' group by 1,a.vrn, b."userFullName"')
            for y in agents_name:
                y.ufn
            print( y.ufn)    
            print ( vrn)
            #need to update code to take latest access token
            access_code=mtd_tokens.objects.raw('select 1 as id,  access_token from pages_mtd_tokens where vrn=\''+ vrn +'\'')
            for x in access_code:
                print (x.access_token)
            endpoint='https://test-api.service.hmrc.gov.uk/agents/'+vrn +'/invitations'      
            headers ={'Authorization': 'Bearer '+ x.access_token,'Accept' :'application/vnd.hmrc.1.0+json','Content-Type':'application/json', }  
            ...
            if r.status_code==204: 
                ...
                return render (request,  'pages/mtd/apis/tuagent.html',
                            {'vat': 'a_last30'    ,
                            'endpoint': y.ufn , #endpoint,
                            'pd2':  str(r.status_code) + '  ' + str(r.reason),
                            #'headers1': headers,
                            'headers1': vrn,
                            'title':  qx  ,  
                            #'clientid': obj.client_id, 
                            'location':'a_last30_none',
                            'errors': x,
                            'au': au,
                            'agents': agents
            ...
            elif r.status_code==400 or r.status_code==403 or r.status_code==401  or r.status_code==406 or r.status_code==501 :        #elif  400 <= r.status_code  <= 499:
                try:   
                    x= r.json()
                except (json.decoder.JSONDecodeError, KeyError, ValueError) as e:
                    x='json  decode failed - issue with app:' +str(e)
                    l1=log1(src_sys_id='last30', status1=' LAST30', 
                    status4='json decode failed',
                    status5=r.status_code,
                    status6=r.reason)
                    l1.save()   
                else:
                    l1=log1(src_sys_id='last30', status1=' LAST30', 
    ...
    def get(self, request):
        ...
        agents=mtd_tokens.objects.raw('select 1 as id, a.vrn, b."userFullName" from pages_mtd_tokens a join pages_testuser b on a.vrn=b.vrn group by 1,a.vrn, b."userFullName"')
        #(' select 1 as id, vrn from pages_mtd_tokens group by 1,vrn ')
...
class a_new_auth(View):
    ...
    def get ( self, request):
        ...
        pd={
            ...
            "planet": 'hmrc',
            "name":'laura3 llc',
            "address": '51 watford avenue,london, ec2a e4ne, london, uk'
    ...
    def post(self, request):
        ...
        if 'upload' in request.POST:
            ...
            post_Data2={
                  "service": [service] , #request.POST.get('service'),  cant get this to work as inout list from form,  so hardcode
            ...
            if r.status_code==204:
                ...
                l1.save()
                xx=r.headers['Location']
                print(" debug: a_authnew: xx=r.headers['Location']" )
                print(xx)
                print("debug: a_Authnew: xx.split('/')")
                zz=xx.split('/')
                print (zz)


                l2=auth_users(src_sys_id=zz[-1],   #potiential bug if len > 20
                    agentarn=str(arn), 
...
class a_get_invite(View):
    def get(self, request):
        return render (request,  'pages/mtd/apis/tuagent.html',
                        {'vat': 'menu MTD AGENT SERVICES -->LAST 30 DAY AUTH'    ,
                        #'endpoint': post_Data2,
                       # 'pd2':  str(r.status_code) + '  ' + str(r.reason),
                        'headers1': 'use --> LAST 30 DAY AUTH  <-- in Menu',
                        'title': 'Post Authentication to get Link for Agent to send to client',
                        'location': 'GET_INVITE',
                        'y1':'y'
    ...
    def post(self, request):
        if 'upload' in request.POST:
           ...
           #       "clientType":request.POST.get('clientType'),
            ...
            headers ={
                'Authorization': 'Bearer '+ x.access_token, #tok.access_token,
                'Accept' :'application/vnd.hmrc.1.0+json',
                'Content-Type':'application/json',
...
class refresh_token(View):
    def get(self, request):
        agents=mtd_tokens.objects.raw('select 1 as id, a.vrn, b."userFullName" from pages_mtd_tokens a join pages_testuser b on a.vrn=b.vrn group by 1,a.vrn, b."userFullName"')
        return render (request,  'pages/mtd/apis/tuagent.html',
                    {'vat': 'refresh access_token : ',# + tok.vrn    ,
                    #'endpoint': endpoint,
                    #'pd2':  str(r.status_code) + '  ' + str(r.reason),
                    #'headers1': 'no headers',
                    #'title': xx,
    ...
    def post(self, request):
        vrn= request.POST.get('vrn1')
        agents_name=mtd_tokens.objects.raw('select 1 as id, a.vrn, b."userFullName" as ufn from pages_mtd_tokens a join pages_testuser b on a.vrn=b.vrn  where a.vrn=\''+ vrn+'\' group by 1,a.vrn, b."userFullName"')
        for y in agents_name:
            y.ufn
        latest_ref_tok=mtd_tokens.objects.raw('select 1 as id,  refresh_token from pages_mtd_tokens a where vrn=\''+vrn +'\' order by created_date desc limit 1')
        #tok=mtd_tokens.objects.latest('id')
        #access_code=mtd_tokens.objects.raw('select 1 as id,  access_token from pages_mtd_tokens where vrn=\''+ vrn +'\'')
        for x1 in latest_ref_tok:
            print(x1.id)
            print(x1.refresh_token)
        endpoint = "https://test-api.service.hmrc.gov.uk/oauth/token"
        ...
        pd ={
            ...
            'client_id':'fQ84pQWgR71bsRWaHRSKRFsh5JQa',
            'grant_type':'refresh_token',
            'refresh_token':x1.refresh_token #tok.refresh_token
        }
        r = requests.post(endpoint,data=pd)
        l1=log1(src_sys_id='refresh token', status1=' refresh token', 
                status3='endpoint=' + endpoint,  
        ...
        #other api end points provide across different keys and some not in json
        #why so inconsistent
        elif  r.status_code  == 400 or r.status_code  == 403  or r.status_code  == 401 :
            ...
            try:
                ...
                xx=x 
                print ('refresh token - json decode error')
                print (xx)
            else:
                xx="error: " + x["error"] + " error description: " + x["error_description"]
                print ( ' error in refresh_token with sc 4xx')
                print ( xx)
            
        else: 
            xx='refresh token failed with status_code' + str( r.status_code)
        return render (request,  'pages/mtd/apis/tuagent.html',
                    {'vat': 'refresh access_token using refresh_token for arn/vrn: ' + vrn,  #tok.vrn    ,
                    'endpoint': endpoint,
                    'pd2':  str(r.status_code) + '  ' + str(r.reason),
                    'headers1': pd, #'no headers',
                    'title': xx,
                    'arn': vrn,
                    'location':'refresh_token',
                    'agentname': y.ufn,
    ...
    def get(self, request):
        ...
        try:
            ...
            l1=log1(src_sys_id='Testuser', status1=' Try  JSON decode error' + str(e),
                status2=str(response.status_code) +' status reason'+ response.reason)
            l1.save()
            ...
            return render(request, 'pages/mtd/apis/tuagent.html',{ 'code': code1,
